...
from meldataset import AudioDataset, mel_spectrogram, get_dataset_filelist, MelDataset
from models import Generator, MultiPeriodDiscriminator, MultiScaleDiscriminator, feature_loss, generator_loss,\
    discriminator_loss, AudioEncoder
...
def train(rank, a, h):
    ...
    if rank == 0:
        validset = AudioDataset(validation_filelist, h.segment_size, h.sampling_rate, h.n_fft, h.num_mels,
                ...
                if steps % a.validation_interval == 0:  # and steps != 0:
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):
                            ...
                            # correct micro-differences (like 189 vs 192), we don't have time for that
                            if y_mel.size(2) > y_g_hat_mel.size(2):
                              y_mel = y_mel[:,:,:y_g_hat_mel.size(2)]
                            elif y_mel.size(2) < y_g_hat_mel.size(2):
                              y_g_hat_mel = y_g_hat_mel[:,:,:y_mel.size(2)]
